...
CephIOAdapterRaw::~CephIOAdapterRaw() {
  ...
  if (m_stats_read_req.load() > 0 && m_stats_read_timer.load() > 0 ) {
    read_speed = m_stats_read_bytes.load() / m_stats_read_timer.load() * 1e-9;
  }
  if (m_stats_write_req.load() > 0 && m_stats_read_timer.load() > 0 ) {
    write_speed = m_stats_write_bytes.load() / m_stats_write_timer.load() * 1e-9;
  }
  BUFLOG("CephIOAdapterRaw::Summary fd:" << m_fd
                                << " nwrite:" << m_stats_write_req << " byteswritten:" << m_stats_write_bytes << " write_ms:"
                                << m_stats_write_timer * 1e-6 << " writemax_ms" << m_stats_write_longest * 1e-6 
                                << " write_MBs:" << write_speed 
                                << " nread:" << m_stats_read_req << " bytesread:" << m_stats_read_bytes << " read_ms:"
                                << m_stats_read_timer * 1e-6 << "  readmax_ms:" << m_stats_read_longest * 1e-6 
                                << " read_MBs:" << read_speed 
